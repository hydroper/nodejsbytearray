...
export default class ByteArray {
    ...
    public set(position: number, value: number) {
        assert(this.position < this._length, 'Setting ByteArray index out of bounds.');
    ...
    private _growIfRequired(length: number): void {
        ...
        this._length += Math.max(0, bytesToAppend);
    }
    ...
    *[Symbol.iterator]() {
    ...
    public readShort(): number {
        assert(this._index + 2 <= this._length, 'Insufficient data available to read.');
    ...
    public readInt(): number {
        assert(this._index + 4 <= this._length, 'Insufficient data available to read.');
    ...
    public readLong(): bigint {
        assert(this._index + 8 <= this._length, 'Insufficient data available to read.');
